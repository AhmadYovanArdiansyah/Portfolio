...
export default function Home() {
	...
	return ( 
		<motion.div initial={{ opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}} transition={{duration: .5}} className="h-full w-full flex flex-col pb-5">
			<div className=" flex-1 flex flex-col items-center md:items-start justify-center mt-32 gap-20">
				<div>
			...
			<div className="h-fit flex items-end pl-[40%]">
				<motion.div initial={{ y: 0, rotate: -135 }} animate={{ y: [-10, 0, -10], rotate: -135 }} transition={{ duration: 2, repeat: Infinity, ease: "linear" }} className="w-6 h-6 transform rotate-[-135deg] origin-center border-l-[3px] blur-[3px] border-t-[3px] border-[#7E00BA]"></motion.div>
			</div>
		</motion.div>
	)
}
